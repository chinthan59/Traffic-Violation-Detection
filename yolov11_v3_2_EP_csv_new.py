...
import tempfile
import json
from src.detector_new import process_car_image
...
def process_detections(image_path):
    """
    Process image and extract violation information using detector_new.py.
    Returns a list of detection results - one per vehicle detected.
    Each vehicle gets its own row with its specific violations.
    
    Uses enhanced dual-model detection (best.pt + car_yolov11_new.pt) with:
    - No_passenger class to prevent false positives
    - Advanced 2-line plate recognition
    - High-precision violation detection
    """
    try:
        # Get API response from detector_new
...
def process_csv(input_csv_path, output_csv_path, start_row=1, max_rows=None):
    ...
    print(f"Using detector: detector_new.py (Enhanced dual-model detector)")
    print(f"Model: best.pt + car_yolov11_new.pt (dual model detection with no_passenger filtering)")
    print("=" * 80)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description='Process images from CSV using detector_new.py (Enhanced dual-model detection with car_yolov11_new.pt)'
    )
    ...
    parser.add_argument('--output', '-o', default='yolov11_v3_EP_new_detection_results.csv',
                       help='Output CSV file path (default: yolov11_v3_EP_new_detection_results.csv)')
    ...
    print("=" * 80)
    print("YOLOv11 CSV Processor - Enhanced Dual-Model Detection")
    print("Detector: detector_new.py (best.pt + car_yolov11_new.pt)")
    print("Features: No_passenger filtering, 2-line plate recognition, ANPR confidence")
    print("=" * 80)
    print()
