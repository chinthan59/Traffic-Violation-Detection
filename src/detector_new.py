...
def ocr_line(img):
    ...
    Returns cleaned uppercase text without spaces and confidence score.
    """
    ...
    try:
        ...
        labels, probs = model_parseq.tokenizer.decode(preds)
        if not labels:
            return "", 0.0
        ...
        return labels[0].replace(" ", "").upper(), conf
    except Exception as e:
        return "", 0.0
...
def predict_plate_with_parseq(image_input):
    ...
    try:
        ...
        full_raw, full_conf = ocr_line(img)
...
def _create_plate_detection(image_cv, bbox, plate_index):
    ...
    plate_text, anpr_confidence = predict_plate_with_parseq(cropped_plate)
    if (not plate_text or not plate_text.strip()) and crnn_predictor:
        try:
            plate_text = crnn_predictor.predict(cropped_plate)
            anpr_confidence = 0.0  # CRNN doesn't provide confidence
        except Exception:
            plate_text = ""
            anpr_confidence = 0.0
...
def process_car_image(file_path: str, filename: str = None, conf_threshold=None, debug=False):
    ...
    for det in detections:
        # CRITICAL FIX: In multi-vehicle scenarios, only keep detections with BOTH number plate AND violations
        # Skip detections with empty violations array (vehicles without violations)
        if not det["violations"]:
            continue
        
        # Create number_plate object
        number_plate_obj = {
            "plate_number": det["plate_number"],
            "plate_bbox": det["bbox"],
            "anpr_confidence": det.get("anpr_confidence", 0.0)
        }
        
        # Add message if plate number is not detected but violations exist
        if (not det["plate_number"] or not det["plate_number"].strip()) and det["violations"]:
            number_plate_obj["message"] = "Number plate not detected"
        
        detection_obj = {
            "number_plate": number_plate_obj,
            "violations": [
                {
                    "type": v["type"],
